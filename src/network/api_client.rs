...
/// Get futures symbols for all three exchanges
pub async fn get_futures_symbols() -> Result<std::collections::HashMap<String, Vec<String>>, Box<dyn std::error::Error + Send + Sync>> {
    let mut all_symbols = std::collections::HashMap::new();

    // Get Binance futures symbols
    if let Ok(binance_symbols) = get_binance_futures_symbols().await {
        all_symbols.insert("BINANCE_FUTURES".to_string(), binance_symbols);
    }

    // Get Bybit futures symbols
    if let Ok(bybit_symbols) = get_bybit_futures_symbols().await {
        all_symbols.insert("BYBIT_FUTURES".to_string(), bybit_symbols);
    }

    // Get OKX futures symbols
    if let Ok(okx_symbols) = get_okx_futures_symbols().await {
        all_symbols.insert("OKX_FUTURES".to_string(), okx_symbols);
    }

    Ok(all_symbols)
}

async fn get_binance_futures_symbols() -> Result<Vec<String>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    if let Some(symbols_array) = data.get("symbols").and_then(|s| s.as_array()) {
        for symbol_info in symbols_array {
            if let Some(symbol) = symbol_info.get("symbol").and_then(|s| s.as_str()) {
                if let Some(status) = symbol_info.get("status").and_then(|s| s.as_str()) {
...
async fn get_bybit_futures_symbols() -> Result<Vec<String>, Box<dyn std::error::Error + Send + Sync>> {
    let client = reqwest::Client::new();
    let response = client
    ...
    if let Some(result) = data.get("result") {
        if let Some(list) = result.get("list").and_then(|l| l.as_array()) {
            for instrument in list {
                if let Some(symbol) = instrument.get("symbol").and_then(|s| s.as_str()) {
                    if let Some(status) = instrument.get("status").and_then(|s| s.as_str()) {
                        if status == "Trading" && symbol.ends_with("USDT") {
                            symbols.push(symbol.to_string());
                        }
                    }
...
async fn get_okx_futures_symbols() -> Result<Vec<String>, Box<dyn std::error::Error + Send + Sync>> {
    let client = reqwest::Client::new();
    let response = client
    ...
    if let Some(data_array) = data.get("data").and_then(|d| d.as_array()) {
        for instrument in data_array {
            if let Some(inst_id) = instrument.get("instId").and_then(|s| s.as_str()) {
                if let Some(state) = instrument.get("state").and_then(|s| s.as_str()) {
                    if state == "live" && inst_id.ends_with("-USDT-SWAP") {
