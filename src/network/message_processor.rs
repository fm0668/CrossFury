...
use crate::json_parser::FastOrderbookParser;
use crate::error_handling::{record_error};
...
fn process_lbank_depth_update(
    ...
    if get_config().features.enable_simd_json {
        ...
        let mut parser = ORDERBOOK_PARSERS.entry(connection_key)
        ...
        if let Some(depth_obj) = value.get("depth") {
            ...
            let (best_ask, best_bid, depth_asks, depth_bids) = parser.value_mut().parse_orderbook(
